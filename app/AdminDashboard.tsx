...
export function GailandAdmin() {
  ...
  const bellCount = (rows.Bookings?.filter(r => r.status === "pending").length || 0) + (rows.Consultations?.filter(r => r.status === "new").length || 0) + (rows.Orders?.filter(r => ["pending", "pending payment"].includes(r.status)).length || 0);
  ...
  return <main className="gb-admin-shell">
    <section className="gb-admin-work">
      <header className="gb-admin-top">
        ...
        <button className={`gb-icon-button ${bellCount ? "has-alerts" : ""}`} onClick={() => go("Bookings")} aria-label={`${bellCount} notifications`}><Bell size={20}/>{bellCount > 0 && <b>{bellCount}</b>}</button>
      </header>
      <div className="gb-admin-nav">
        <div className="gb-view-switcher"><button className="gb-view-trigger" onClick={() => setViewMenuOpen(v => !v)} aria-expanded={viewMenuOpen}><span>{view}</span><ChevronDown size={15}/></button>{viewMenuOpen && <div className="gb-view-menu">{nav.map(item => <button key={item.label} className={view === item.label ? "active" : ""} onClick={() => go(item.label)}><item.icon size={15}/><span>{item.label}</span>{(["Bookings","Consultations","Orders"].includes(item.label)) && <b>{rows[item.label]?.length || 0}</b>}</button>)}</div>}</div>
