...
export function GailandAdmin() {
  const [authenticated, setAuthenticated] = useState(() => !isSupabaseConfigured && typeof window !== "undefined" && sessionStorage.getItem("gailand-admin-session") === "active");
  ...
  const login = async (form: FormData) => {
    ...
    const localMatch = auth.local && user === (process.env.NEXT_PUBLIC_ADMIN_USERNAME || "admin").toLowerCase() && password === (process.env.NEXT_PUBLIC_ADMIN_PASSWORD || "gailand2026admin");
    if ((!auth.local && !auth.error) || localMatch) {
      sessionStorage.setItem("gailand-admin-session", "active"); setAuthenticated(true); setLoginError("");
    } else setLoginError(auth.error || "Those details don’t match the staff account.");
  };
  ...
  return <main className="gb-admin-shell">
    <section className="gb-admin-work">
      ...
      {loading && <div className="gb-loading">Refreshing Gailand records…</div>}{dataError&&<div className="gb-data-error" role="alert">{dataError}<button onClick={()=>setDataError("")}>×</button></div>}
